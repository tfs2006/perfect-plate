...
(function () {
  ...
  ready(() => {
    ...
    function goToStep(i) {
      [1, 2, 3].forEach((s) => {
        const p = $(`step-${s}`);
        if (p) p.classList.toggle('active', s === i);
        const ind = $(`step-${s}-indicator`);
        if (ind) {
          ind.style.opacity = (s === i) ? '1' : '.5';
          const b = ind.querySelector('div');
          if (b) b.classList.toggle('bg-emerald-500', s === i);
        }
      });
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
    // Inline fallback compatibility if HTML ever uses onclick="nextStep(2)"
    ...
    // Loader helpers (remove 'hidden' just in case)
    function showLoader() {
      if (!loader) return;
    ...
    function loadState() {
      try {
        ...
        if (Array.isArray(d.diet)) {
          d.diet.forEach(v => document.querySelector(`input[name="diet"][value="${v}"]`)?.setAttribute('checked', 'checked'));
        }
      } catch {}
    }
    loadState();
    ...
    async function secureApiCall(path, payload) {
      ...
      try {
        ...
        throw new Error(`Failed to reach ${url}. ${
          /Failed to fetch/i.test(String(err)) ? 'Possible CORS or wrong API_BASE. Check ALLOWED_ORIGIN on Netlify and the URL.' : String(err)
        }`);
      }
    }
